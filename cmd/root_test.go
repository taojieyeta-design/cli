...
// baseline for the errcompat teardown: it pins the typed envelope and exit code
// that the dispatcher produces for the two source-of-truth auth/config error
// shapes. The construction sites move from the dispatcher boundary to the
// origin, but every asserted field below must remain byte-for-byte unchanged.
