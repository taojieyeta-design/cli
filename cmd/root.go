...
func handleRootError(f *cmdutil.Factory, err error) int {
	...
	// Anything reaching here is a cobra usage error (missing required flag,
	// unknown command, bad args); RunE and flag-parse errors are already typed
	// above. Classify as invalid_argument so it shares the typed envelope and
	// exit 2 of the flag/subcommand guards instead of a plain "Error:" line,
	// which also preserves any pending deprecation notice.
	usageErr := errs.NewValidationError(errs.SubtypeInvalidArgument, "%s", err.Error())
	output.WriteTypedErrorEnvelope(errOut, usageErr, string(f.ResolvedIdentity))
	return output.ExitCodeOf(usageErr)
}
...
func unknownSubcommandRunE(cmd *cobra.Command, args []string) error {
	...
	// The offending token is a positional subcommand name, not a flag, so it
	// is not recorded as a param; the suggestions are already folded into hint.
...
func flagDidYouMean(c *cobra.Command, ferr error) error {
	...
	// suggestions are folded into hint; valid_flags is recoverable via --help.
	// Use Params (not the scalar Param) so this path matches the
	// before-a-subcommand guard and carries a per-flag reason for agents.
