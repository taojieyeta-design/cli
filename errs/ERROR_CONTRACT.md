...
*why*, one entry each. Each `errs.InvalidParam` is `{Name, Reason string}`:
`Name` identifies the parameter, `Reason` states why it failed. This is
the CLI's rendering of the RFC 7807 `invalid-params` extension member
