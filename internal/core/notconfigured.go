...
// malformed file (parse / invalid format) rather than an absent or
// inaccessible one. Malformed files map to the invalid_config subtype so the
// user is told to fix the file instead of re-running init.
func isMalformedConfigError(err error) bool {
	lower := strings.ToLower(err.Error())
	return strings.Contains(lower, "parse") || strings.Contains(lower, "invalid")
}
